...
<script lang="ts">
  ...
  import { PALETTE_TOGGLE_EVENT, games, modules, tools } from "../constants";

  export let open = false;
  let projects = [...tools, ...games, ...modules];
  let index = new Flexsearch.Index({ tokenize: "full" });
  let query: string;
  $: results = index.search(query).map((i) => projects[Number(i)]);
