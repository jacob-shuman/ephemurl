<script lang="ts">
  ...
  const anchorClass = tw(
    "cursor-pointer w-12 h-12 flex justify-center items-center rounded",
    "transition-all duration-100 ease-out",
    "hover:bg-fg-500 hover:dark:bg-fg-dark-500 hover:text-fg-100 hover:dark:fg-dark-100",
...
{#if tooltip}
  <Tooltip message={tooltip}>
    <T.Trigger asChild let:builder>
      <a use:builder.action {...builder} {href} class={anchorClass}>
        <slot />
      </a>
    </T.Trigger>
  </Tooltip>
